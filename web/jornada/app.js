...
function arquivoDocumento(doc) {
  return doc.arquivoUrl || doc.arquivo_url || '';
}

function botaoVerDocumento(doc) {
  const url = arquivoDocumento(doc);
  if (!url) {
    return '<button class="btn btn-sm btn-disabled" disabled>📎 Sem arquivo</button>';
  }

  return `<button class="btn btn-sm btn-outline" onclick="abrirDocumento('${encodeURIComponent(url)}')">📎 Ver</button>`;
}

function abrirDocumento(url) {
  if (!url) {
    showToast('Este documento não possui arquivo anexado.', 'error');
    return;
  }

  const arquivoUrl = decodeURIComponent(url);
  const viewer = document.getElementById('document-viewer-body');
  const modal = document.getElementById('modal-documento-viewer');
  const cleanUrl = arquivoUrl.split('?')[0].toLowerCase();
  const isPdf = cleanUrl.endsWith('.pdf');

  viewer.innerHTML = isPdf
    ? `<iframe class="document-frame" src="${arquivoUrl}" title="Documento"></iframe>`
    : `<img class="document-image" src="${arquivoUrl}" alt="Documento">`;

  modal.classList.add('open');
}

function emailPorMatricula(matricula) {
  return `${matricula.trim()}@app.com`;
}

function dataFirestoreParaIso(valor) {
  if (!valor) return '';
...
function timestampServidor() {
  return firebase.firestore.FieldValue.serverTimestamp();
}

function mapColaborador(doc) {
  const data = doc.data();
  return {
    id: doc.id,
...
function mapDocumento(doc) {
  ...
  return {
    ...
    dataValidade: dataFirestoreParaIso(data.dataValidade),
    dataEmissao: dataFirestoreParaIso(data.dataEmissao),
    status: data.status || calcStatus(dataFirestoreParaIso(data.dataValidade)),
    arquivoUrl: data.arquivoUrl || '',
    ...data
  };
}

function mapEquipamento(doc) {
  const data = doc.data();
...
function mapEmpresaDoc(doc) {
  const data = doc.data();
  return {
    id: doc.id,
    titulo: data.titulo || data.nome || 'Documento',
    categoria: data.categoria || data.tipo || 'Geral',
    dataValidade: dataFirestoreParaIso(data.dataValidade || data.data_vencimento),
    dataEmissao: dataFirestoreParaIso(data.dataEmissao || data.data_emissao),
    status: data.status || calcStatus(dataFirestoreParaIso(data.dataValidade || data.data_vencimento)),
